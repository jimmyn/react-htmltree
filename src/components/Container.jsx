...
export default class Container extends Component {
        ...
        if (node.children) {
          if (node.children.length) {
            node.children = node.children.filter(
              child => child.type !== 'text' || child.data.trim().length
            );
  ...
  onUpdate(e, component, type, data) {
    ...
    const element = origin
      ? selector.match('>') ? origin.querySelectorAll(selector)[0] : origin
      : {
    ...
    let updater = null; // toggle: (value) => !value
      ...
      case 'toggleExpand':
        ...
        if (e.altKey && e.ctrlKey) {
          return this.setState({
            root: root.setIn(
              [...node.get('keyPath').toJS()],
              setDeep(node, 'children', ['state', 'expanded'], true)
            )
          });
        }
        // TODO:
